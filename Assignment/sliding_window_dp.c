...
static void send_data_frame(MSG *msg, size_t len, int seq, int dest)
{
    ...
    f.link = routing_table[nodeinfo.nodenumber][f.destnode];
    ...
    CHECK(CNET_write_physical_reliable(f.link, (char *) &f, &size));
}
...
static void send_ack_frame(int seq, int type, int link)
{
    ...
    if(type == REJ){
        f.seq = -seq;
        printf("Reject transmitted,\tSequence No: %d, \tFrom Node Number: %d,\t", f.seq, f.srcnode);
    }
    else{
        f.seq = seq;
...
static void middle_man_send(DATA_FRAME f, int dest){
    ...
    f.checksum = 0;
    f.link = routing_table[nodeinfo.nodenumber][dest];
    size = DATA_FRAME_SIZE(f);
    f.checksum  = CNET_ccitt((unsigned char *) &f, (int)size);
...
static EVENT_HANDLER(physical_ready)
{
    ...
    if(CNET_ccitt((unsigned char*) &f, len) != checksum)
    {
        ...
        send_ack_frame(f.link, last_ack_sent + 1, REJ);
        return;
    }
    ...
    if(f.type == DATA){
        if(f.destnode == nodeinfo.nodenumber){
            ...
            if(i == f.seq)
                send_ack_frame(f.link, f.seq + 1, RR);
        }
        else
